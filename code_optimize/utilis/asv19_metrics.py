...
ASV19_SCORES_DIR = PROJECT_ROOT / "official_scores" / "ASVspoof2019_LA"
LABEL_FILE = ASV19_SCORES_DIR / "ASVspoof2019.LA.cm.eval.trl.txt"
ASV_LABEL = ASV19_SCORES_DIR / "ASVspoof2019.LA.asv.eval.gi.trl.scores.txt"


def configure_asv19_assets(official_scores_dir: str | Path) -> None:
    """Point the copied official ASVspoof2019 metric code at its score assets."""

    global ASV19_SCORES_DIR, LABEL_FILE, ASV_LABEL
    scores_dir = Path(official_scores_dir).expanduser().resolve()
    if scores_dir.name != "ASVspoof2019_LA" and (scores_dir / "ASVspoof2019_LA").is_dir():
        scores_dir = scores_dir / "ASVspoof2019_LA"
    required = (
        scores_dir / "ASVspoof2019.LA.cm.eval.trl.txt",
        scores_dir / "ASVspoof2019.LA.asv.eval.gi.trl.scores.txt",
    )
    missing = [path for path in required if not path.is_file()]
    if missing:
        raise FileNotFoundError(
            "Missing ASVspoof2019 official score asset(s): "
            + ", ".join(str(path) for path in missing)
        )
    ASV19_SCORES_DIR = scores_dir
    LABEL_FILE, ASV_LABEL = required


def get_asv19_cost_model():
    p_spoof = 0.05
    return {
        'Pspoof': p_spoof,
        'Ptar': (1 - p_spoof) * 0.99,
...
def load_asv19_asv_operating_point():
...
def load_asv19_cm_scores(score_file):
    scores = {}
    with open(score_file, 'r', encoding="utf-8") as f:
        for raw_line in f:
            line = raw_line.strip().split()
            if len(line) > 3:
                scores[line[0]] = float(line[3])
    return scores


def compute_asv19_cm_metrics(target_score, nontarget_score, asv_operating_point=None):
    target_score = np.array(target_score, dtype=np.float64)
    nontarget_score = np.array(nontarget_score, dtype=np.float64)
    eer_cm, _ = compute_eer(target_score, nontarget_score)

    if asv_operating_point is None:
        asv_operating_point = load_asv19_asv_operating_point()
    tDCF_curve, _ = compute_tDCF(
        target_score,
        nontarget_score,
        asv_operating_point["pfa_asv"],
        asv_operating_point["pmiss_asv"],
        asv_operating_point["pmiss_spoof_asv"],
        get_asv19_cost_model(),
        False,
    )
...
def eerandtdcf(score_file):
    target = []
    nontarget = []
    target_score = []
    nontarget_score = []
    protocol = load_asv19_cm_protocol()
    score = load_asv19_cm_scores(score_file)

    for wav_id, item in protocol.items():
        if item["label"] == "spoof":
    ...
    return compute_asv19_cm_metrics(target_score, nontarget_score)


def attack_level_eerandtdcf(score_file):
    protocol = load_asv19_cm_protocol()
    score = load_asv19_cm_scores(score_file)
    asv_operating_point = load_asv19_asv_operating_point()
    ...
    bonafide_scores = [
        score[wav_id]
        for wav_id, item in protocol.items()
        if item["label"] == "bonafide" and wav_id in score
    ]
    attack_to_scores = {}
    for wav_id, item in protocol.items():
        if item["label"] != "spoof" or wav_id not in score:
            continue
        attack_to_scores.setdefault(item["attack"], []).append(score[wav_id])

    rows = []
    for attack in sorted(attack_to_scores):
        spoof_scores = attack_to_scores[attack]
        eer_cm, min_tDCF = compute_asv19_cm_metrics(
            bonafide_scores,
            spoof_scores,
            asv_operating_point=asv_operating_point,
        )
        rows.append(
            {
                "attack": attack,
                "bonafide_trials": len(bonafide_scores),
                "spoof_trials": len(spoof_scores),
                "eer": eer_cm,
                "mtdcf": min_tDCF,
            }
        )
...
__all__ = [
    "attack_level_eerandtdcf",
    "configure_asv19_assets",
    "eerandtdcf",
]
