...
from utilis.asv19_metrics import (
...
PROJECT_ROOT = CODE_OPTIMIZE_ROOT.parent
OUTPUTS_ROOT = CODE_OPTIMIZE_ROOT / "outputs"
DEFAULT_ASV19_SCORES_DIR = PROJECT_ROOT / "official_scores" / "ASVspoof2019_LA"
...
def resolve_results_dir(path: str | Path) -> Path:
    ...
    if path.is_absolute():
        path = path.resolve()
    elif path.parts and path.parts[0] == CODE_OPTIMIZE_ROOT.name:
        path = (PROJECT_ROOT / path).resolve()
    elif path.parts and path.parts[0] == OUTPUTS_ROOT.name:
        path = (CODE_OPTIMIZE_ROOT / path).resolve()
    else:
...
def discover_prediction_files(
    ...
    if file_names:
        ...
        for file_name in file_names:
            ...
            if not path.name.lower().startswith(PREDICTION_PREFIX):
                raise ValueError(
                    f"ASV19 prediction file must start with {PREDICTION_PREFIX!r}: {path.name}"
                )
    ...
    if not predictions:
        raise FileNotFoundError(f"No ASV19 prediction csv files selected under {results_dir}.")
    return predictions
...
def write_asv19_cm_score(prediction_csv: Path, score_path: Path) -> Path:
    ...
    if not np.isfinite(scores).all():
        raise FloatingPointError(f"{prediction_csv} contains non-finite bonafide_score values.")

    score_path.parent.mkdir(parents=True, exist_ok=True)
    with score_path.open("w", encoding="utf-8") as handle:
        for row in df.itertuples(index=False):
            label_name = "bonafide" if int(row.label) == 0 else "spoof"
            handle.write(f"{row.utterance_id} - {label_name} {float(row.bonafide_score)}\n")
    return score_path
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--official-scores-dir",
        default=str(DEFAULT_ASV19_SCORES_DIR),
        help="Directory containing the ASVspoof2019 LA official score assets.",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    configure_asv19_assets(args.official_scores_dir)
